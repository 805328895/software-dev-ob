...
- 用例模板：`{baseDir}/../../templates/测试用例.md`（与 Claude 版共享）
...
1. 用 `$CLI read file="REQ-XXX-..."` 或文件读取读需求，拿 reqId/title/module/domain + 正文「涉及接口(板块4)」「数据模型(板块5)」「改动点」
...
3. 若 pmProjectCode 为空 → 用 Ask user（`ask_user` 能力）让用户手填测试地址
...
3. 写到 `<vaultPath>/<kbDir>/tests/REQ-{编号}-{slug}-用例.md`
