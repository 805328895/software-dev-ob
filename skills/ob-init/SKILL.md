...
> config 字段定义详见 `references/config-spec.md`。

## config 位置约定（重要）

config 跟**当前工作目录**走，每个仓库各一份，不跟插件目录：

- 文件名：`.software-dev-ob.config.json`（点开头，建议加进项目 `.gitignore`）
- 位置：git 仓库根（`git rev-parse --show-toplevel`）；非 git 项目就放当前工作目录（`pwd`）

> 这样多项目各一份 config，互不串。`<skill目录>/` 只放 skill 文件本身，不放运行配置。
...
读取 `<skill目录>/templates/` 下模板，把 `{{kbDir}}` 替换为实际目录名、`{{date}}` 替换为当天日期，用 `Write` 写入 vault：
...
用 `Write` 写到**当前工作目录**的 `.software-dev-ob.config.json`（git 仓库根；无 git 则 cwd）：
...
- 模板写入失败 → 只补缺失文件，不全量重建
- config 写失败 → 提示手动补 config（流程已建好骨架）

## 不允许的情况

- ❌ 不经对话框确认就自行假设目录名
- ❌ 目录已存在时仍覆盖现有文件
- ❌ 不写 `.software-dev-ob.config.json` 就结束
- ❌ 把 config 写进插件目录（skill 目录）而非当前工作目录
