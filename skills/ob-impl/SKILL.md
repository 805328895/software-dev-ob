...
> 开发规范详见 `references/dev-conventions.md`（纯指针，指向仓库自身 CLAUDE.md）；流转链路详见 `references/status-flow.md`。

## 引用加载策略

- 开发规范：读 `references/dev-conventions.md`（纯指针，指向仓库自身 CLAUDE.md，不内联任何语言规范）
...
1. 用 `$CLI read file="REQ-XXX-..."` 或 Read 读 `<vaultPath>/<kbDir>/00-需求/REQ-XXX-*.md`，拿 frontmatter（reqId/title/module/status）+ 正文（涉及接口板块4 / 数据模型板块5 / 改动点 checkbox）
...
> 实现过程中若发现需求接口/表/字段与代码现状矛盾，停下来 `AskUserQuestion` 确认，不擅自改需求或绕过。
